...
!       print*, "K = ",K
  ...
  subroutine backSub(U, B2, X)
    ...
    if (U(size(U,1),size(U,2)) < tol) then !check for singular matrix
...
!    real(dp), intent(inout) :: A_diag(:,:)
    ...
    do j = 1, size(A,2)  !loop over columns
       ...
       if (A(j,j) .LE. tol) then  !Matrix A must be SPD, symmetric positive definite (all diagonal elements > 0)
          sTest = .true.
...
!       print*, "A(j,j) for j =",j," is ", A(j,j)
  ...
  subroutine Cholesky_backSub(A, B, X)
    ...
    !First, forward substitution, solving Ly = b
    !do k = 1, size(B,2) !loop over columns of right hand side matrix
    Y = B
    sum = 0.0
    do i = 1, size(A,1)
...
!    print*, "shape B_n",shape(B_n)
  ...
  subroutine inv_Iter(A,ews,x,eye)
    ...
    B = (A - mu*eye)
!    call LU_pp(B,size(B,1),sTest,s) ! factorize (A-mu*eye) outside of while loop
    call gausElim_pp(B,x,shape(B),shape(x),sTest)
    do while (norm2(r(:,1)) > tol)
!       call LU_backSub(B,size(B,1),x,s,y) !perform LU_backSub to solve for y = (A-mu*eye)^{-1}(x)
       call backSub(B,x,y)
       y = y / norm2(y)
      ! print*, "y = ",y
       r = y - x
...
!    print*, "l = ",l
  ...
  subroutine g_jacobi(A,b,x,tol,count,upper_tol)
    ...
    R = A
    do i = 1, size(A,1)
       R(i,i) = 0.d0     !for the R matrix, which is A with zeros on diagonal
